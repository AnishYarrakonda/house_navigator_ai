...
> **Goal:** Map renders with the custom warm-dark style + seeded nodes + **one pre-baked glowing journey**. If everything else breaks, this alone wows on load.
...
2. **MapLibre + warm-dark style.** Full-bleed map as the home screen (not a tab), centered on San Francisco. Custom warm-dark basemap style.
...
- `npm run dev` opens to a warm-dark SF map with pins + one animated glowing route.
